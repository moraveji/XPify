...
async function loadRelease(releaseId) {
  ...
  const albumNotesRaw = await fetchOptionalText(`${releaseId}/album-notes.txt`);
  if (albumNotesRaw) {
  ...
  if (!albumCoverSrc) {
    const albumCoverUrlOverride = await fetchOptionalText(`${releaseId}/album-cover.url`);
...
async function discoverTracks(releaseId) {
  ...
  for (let i = 1; i <= 50; i += 1) {
    ...
    if (textContent) {
      ...
      track.subtitle = parsed.subtitle;
      track.bodyHtml = parsed.bodyHtml;
      track.links = parsed.links;
      if (parsed.image) track.image = parsed.image;
      if (parsed.audio) track.audio = parsed.audio;
      if (parsed.kenburns) track.kenburns = parsed.kenburns;
    }
