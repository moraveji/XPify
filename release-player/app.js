...
function setupSplitter() {
  ...
  const applyWidth = (px, limits = bounds()) => {
  ...
  const clampWidth = (value, limits = bounds()) => {
    const { min, max } = limits;
  ...
  const applyInitialWidth = () => {
    ...
    if (limits.rect.width <= 0) {
      ...
      initAttempts += 1;
      if (initAttempts < 10) {
        requestAnimationFrame(applyInitialWidth);
      }
      return;
    }

    const numericStored = stored ? parseFloat(stored) : NaN;
    const fallback = clampWidth(800, limits);
    const initial = clampWidth(
      Number.isFinite(numericStored) ? numericStored : fallback,
      limits,
    );
    applyWidth(initial, limits);
  };
